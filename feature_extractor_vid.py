...
def feature_extractor():
	...
	for video_path, file_name in zip(path_list, video_list): 
		...
		while True: 
		    ...
		    if not ret:
		        break
		    cv2.imwrite(os.path.join(frame_path, 'pic_%05d.jpg'%num), img)  #写出视频图片.jpg格式
		    num = num + 1
		capture.release()

		print('Extracting features ...')
		total_frames = num - 1	# len(os.listdir(frame_path))
		print('total_frames')
		if total_frames == 0:
		...
		valid_frames = int(total_frames / nb_frames * nb_frames)	#每16帧一组，可用帧数
		n_feat = int(valid_frames / nb_frames)	#多少组帧，多少组特征
		n_batch = int(n_feat / BATCH_SIZE)	#一批30组，多少批
		...
		for i in range(n_batch-1):
			...
			for j in range(BATCH_SIZE):
				clip = []
				clip = np.array([resize(io.imread(os.path.join(frame_path, 'pic_{:05d}.jpg'.format(k))), output_shape=(resize_w, resize_h), preserve_range=True) for k in range((i*BATCH_SIZE+j) * nb_frames+1, min((i*BATCH_SIZE+j+1) * nb_frames+1, valid_frames+1))])
		...
		for j in range(n_feat-(n_batch-1)*BATCH_SIZE):
			clip = []
			print(range(((n_batch-1)*BATCH_SIZE+j) * nb_frames+1, min(((n_batch-1)*BATCH_SIZE+j+1) * nb_frames+1, valid_frames+1)))
			clip = np.array([resize(io.imread(os.path.join(frame_path, 'pic_{:05d}.jpg'.format(k))), output_shape=(resize_w, resize_h), preserve_range=True) for k in range(((n_batch-1)*BATCH_SIZE+j) * nb_frames+1, min(((n_batch-1)*BATCH_SIZE+j+1) * nb_frames+1, valid_frames+1))])
		...
		for feature in features:
			...
			feature_file.write('\n')
		'''
		fgroup = f.create_group(file_name)
